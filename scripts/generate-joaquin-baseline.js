 ...
 *
 * Character: joaquin
 *   - Stocky basketball player, 66 in, 160 lbs
 *   - Outfit: "Lucky Trucker" black shirt, blue jeans
 *   - Style: 16-bit pixel art, GBA style
 *   - Portrait: data/assets/joaquinfull.png
 *   - Angles: joaquin-angle-0..7.png (use angle-2 = side-L as char ref)
 *
 * Animations:
 *   1. joaquin-idle   — 4 frames, 720x180,  loop: true,  no ball, fps 6
 *   2. joaquin-walk   — 8 frames, 1440x180, loop: true,  no ball, fps 10
 *
 * Padding fix: 160x160-within-180x180 (proven fix from viv-idle/bron-test/z precedent)
 * Model: gemini-2.5-flash-image
 * QC threshold: 80/100 to ACCEPT
 *
 * Output: data/assets/joaquin-idle.png, data/assets/joaquin-walk.png
 */
'use strict';
...
const STATUS_FILE   = '/tmp/joaquin-baseline-status.json';
const MODEL_ID      = 'gemini-2.5-flash-image';
const QC_THRESHOLD  = 80;

const CHAR_REF = path.join(ASSETS_DIR, 'joaquin-angle-2.png'); // side-L profile
...
function buildIdlePrompt() {
  return [
    ...
    'outfit, and proportions. The character is a stocky, shorter player wearing a black',
    '"Lucky Trucker" shirt and blue jeans. This is a side-profile view of the character.',
...
async function runAnimation({ name, frames, expectedWidth, prompt, refs }) {
  const rawPath   = path.join(RAW_DIR, `joaquin-${name}-raw.png`);
  const finalPath = path.join(ASSETS_DIR, `joaquin-${name}.png`);
  const framesDir = path.join(ASSETS_DIR, `joaquin-${name}-frames`);
  const tempDir   = path.join(BASE_DIR, `data/.tmp-joaquin-${name}`);

  console.log(`\n====== Generating joaquin-${name} (${frames} frames) ======`);
  ...
  recordCost(MODEL_ID, 'strip', '2K', 1, { character: 'joaquin', animation: name });
  ...
  // QC evaluation — use frames from framesDir
...
async function main() {
  ...
  // ── 1. joaquin-idle ─────────────────────────────────────────────────────────
  try {
    writeStatus({ status: 'generating joaquin-idle' });
    ...
    console.error('ERROR joaquin-idle:', err.message);
    results.idle = { name: 'idle', qcStatus: 'ERROR', error: err.message };
  }

  // ── 2. joaquin-walk ─────────────────────────────────────────────────────────
  try {
    writeStatus({ status: 'generating joaquin-walk' });
    ...
    console.error('ERROR joaquin-walk:', err.message);
  ...
  for (const [k, v] of Object.entries(results)) {
    console.log(`  joaquin-${k}: ${v.qcStatus} score=${v.qcScore ?? 'N/A'} dims=${v.width ?? '?'}x${v.height ?? '?'}`);
  }
  ...
  if (accepted.length > 0) {
    try {
      ...
      for (const r of accepted) {
        ...
        contract.characters['joaquin'].animations[r.name] = {
          frames: r.frames,
          frameWidth: 180,
          frameHeight: 180,
          stripWidth: r.width,
          ...
          qcNote: `TASK-6008 — joaquin baseline generation. ${r.frames} frames, ${r.width}x${r.height} confirmed.`,
          file: `data/assets/joaquin-${r.name}.png`,
        };
      }
      ...
      console.log(`\nUpdated animation-contract.json: added ${accepted.map(r => r.name).join(', ')} to characters.joaquin`);
    } catch (err) {
