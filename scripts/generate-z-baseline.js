#!/usr/bin/env node
/**
 * TASK-6006: Generate z baseline animations — idle + walk
 *
 * Character: z
 *   - Athletic basketball player, 72 in, standard build
 *   - Portrait: data/assets/zfull.png
 *   - Angles: z-angle-0..7.png (use angle-2 = side-L as char ref)
 *   - Ball refs: z-ball-*.png
 *
 * Animations (z-dribble ALREADY ACCEPTED 100/100 — SKIP):
 *   1. z-idle   — 4 frames, 720x180,  loop: true,  no ball, fps 6
 *   2. z-walk   — 8 frames, 1440x180, loop: true,  no ball, fps 10
 *
 * Padding fix: 160x160-within-180x180 (proven fix from viv-idle/bron-test)
 * Model: gemini-2.5-flash-image
 * QC threshold: 80/100 to ACCEPT
 *
 * Output: data/assets/z-idle.png, data/assets/z-walk.png
 */
'use strict';
...
const STATUS_FILE   = '/tmp/z-baseline-status.json';
const MODEL_ID      = 'gemini-2.5-flash-image';
const QC_THRESHOLD  = 80;

const CHAR_REF = path.join(ASSETS_DIR, 'z-angle-2.png'); // side-L profile
...
async function runAnimation({ name, frames, expectedWidth, prompt, refs }) {
  const rawPath   = path.join(RAW_DIR, `z-${name}-raw.png`);
  const finalPath = path.join(ASSETS_DIR, `z-${name}.png`);
  const framesDir = path.join(ASSETS_DIR, `z-${name}-frames`);
  const tempDir   = path.join(BASE_DIR, `data/.tmp-z-${name}`);

  console.log(`\n====== Generating z-${name} (${frames} frames) ======`);
  ...
  recordCost(MODEL_ID, 'strip', '2K', 1, { character: 'z', animation: name });
...
async function main() {
  ...
  console.log('NOTE: z-dribble is ACCEPTED 100/100 — SKIPPING (task spec)');

  const results = {};

  // ── 1. z-idle ─────────────────────────────────────────────────────────────
  try {
    writeStatus({ status: 'generating z-idle' });
    ...
    console.error('ERROR z-idle:', err.message);
    results.idle = { name: 'idle', qcStatus: 'ERROR', error: err.message };
  }

  // ── 2. z-walk ─────────────────────────────────────────────────────────────
  try {
    writeStatus({ status: 'generating z-walk' });
    ...
    console.error('ERROR z-walk:', err.message);
  ...
  for (const [k, v] of Object.entries(results)) {
    console.log(`  z-${k}: ${v.qcStatus} score=${v.qcScore ?? 'N/A'} dims=${v.width ?? '?'}x${v.height ?? '?'}`);
  }
  ...
  if (accepted.length > 0) {
    try {
      ...
      if (!contract.characters['z']) contract.characters['z'] = { animations: {} };
      if (!contract.characters['z'].animations) contract.characters['z'].animations = {};

      for (const r of accepted) {
        const isIdle = r.name === 'idle';
        const isWalk = r.name === 'walk';
        contract.characters['z'].animations[r.name] = {
          frames: r.frames,
          frameWidth: 180,
          frameHeight: 180,
          stripWidth: r.width,
          ...
          qcNote: `TASK-6006 — z baseline generation. ${r.frames} frames, ${r.width}x${r.height} confirmed.`,
          file: `data/assets/z-${r.name}.png`,
        };
      }
      ...
      console.log(`\nUpdated animation-contract.json: added ${accepted.map(r => r.name).join(', ')} to characters.z`);
    } catch (err) {
