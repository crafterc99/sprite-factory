...
function register(router, { ASSETS_DIR, RAW_DIR, runWithConcurrency, json, parseBody }) {
  ...
  router.post('/api/generate/angles', async (req, res) => {
    ...
    const modelId = body.model && ['gemini-3-pro-image-preview', 'gemini-3-pro-image-preview', 'gemini-2.5-flash-image'].includes(body.model)
      ? body.model
  ...
  // Body: { characters: [...], animation: "dribble", model?: "gemini-3-pro-image-preview" }
  ...
  router.post('/api/pipeline/fill-gaps', async (req, res) => {
    try {
      const body = await parseBody(req);
      const modelId = (body.model && ['gemini-3-pro-image-preview', 'gemini-3-pro-image-preview', 'gemini-2.5-flash-image'].includes(body.model))
        ? body.model : 'gemini-3-pro-image-preview';
