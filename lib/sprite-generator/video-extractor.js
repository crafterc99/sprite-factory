...
async function extractFrames(videoPath, outputDir, opts = {}) {
  ...
  // Output 1: full-resolution frames (max-quality JPEG — these feed the
  // player cutout AI, so no visible compression is acceptable)
  let filter = `fps=${fps}`;
  if (opts.scale) {
    filter += `,scale=${opts.scale}:-2:flags=lanczos`;
  }
  args.push('-vf', filter, '-q:v', '1', path.join(outputDir, 'frame-%04d.jpg'));
