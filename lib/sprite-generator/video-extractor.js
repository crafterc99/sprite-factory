...
async function extractFrames(videoPath, outputDir, opts = {}) {
  ...
  // Output 1: full-resolution frames (high-quality JPEG)
  let filter = `fps=${fps}`;
  if (opts.scale) {
    filter += `,scale=${opts.scale}:-2:flags=lanczos`;
  }
  args.push('-vf', filter, '-q:v', '2', path.join(outputDir, 'frame-%04d.jpg'));

  // Output 2: gallery thumbnails from the same decode pass
  args.push('-vf', `fps=${fps},scale=-2:200`, '-q:v', '6', path.join(outputDir, 'thumb-%04d.jpg'));
