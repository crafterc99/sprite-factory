...
class NanaBananaClient {
  ...
  async generateSingleFrame(prompt, poseFramePath, characterRef, opts = {}) {
    ...
    // Image 3 = style anchor — frame 0 of this run, passed for frames 1+ to lock rendering consistency
    if (opts.styleAnchorPath && fs.existsSync(opts.styleAnchorPath)) {
      referenceImages.push(opts.styleAnchorPath);
    }
