    ...
    for (let mi = 0; mi < chain.length; mi++) {
      ...
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
          ...
          const isOverloaded = err.status === 503 || err.code === 503 ||
            errMsg.includes('503') || errMsg.includes('UNAVAILABLE') ||
            errMsg.includes('high demand') || errMsg.includes('overloaded') ||
            errMsg.includes('Service Unavailable');

          if (isRateLimit || isTimeout || isOverloaded) {
            // Rate limited, timed out, or model overloaded — skip to next model in chain
            const reason = isTimeout ? 'Timeout' : isOverloaded ? '503 overloaded' : 'Rate limit';
            console.log(`[NanaBanana] ${reason} on ${activeModel} (attempt ${attempt + 1}), trying next model…`);
            break;
          }
