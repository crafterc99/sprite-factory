...
const MODEL     = 'gemini-3.1-flash-image-preview'; // Nano Banana 2 — default
const MODEL_PRO = 'gemini-3-pro-image-preview';     // Nano Banana Pro — best quality
...
const MODEL_FALLBACK_CHAIN = [MODEL_PRO, MODEL, MODEL_FALLBACK];
...
class NanaBananaClient {
  ...
  async generate(prompt, opts = {}) {
    ...
    const TIMEOUT_MS = opts.timeoutMs || 90000;
    ...
    for (let mi = 0; mi < chain.length; mi++) {
      ...
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
          ...
          const isRetryable = isRateLimit || err.status === 503 || err.code === 503 ||
          ...
          if (isRateLimit) {
            // Rate limited on this model — break inner loop to try next model in chain
            console.log(`[NanaBanana] Rate limited on ${activeModel} (attempt ${attempt + 1})`);
            break;
          }
