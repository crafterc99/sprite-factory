...
async function smartSelect(framePaths, count, opts = {}) {
  ...
  console.log(`  Weights: ball=${ballWeight} motion=${motionWeight} sharp=${sharpWeight} spacing=${spacingWeight}`);
  ...
  for (let i = 0; i < framePaths.length; i++) {
    ...
    if ((i + 1) % 10 === 0 || i === framePaths.length - 1) {
      process.stdout.write(`    ${i + 1}/${framePaths.length}\r`);
    }
  }
  console.log(`    ${framePaths.length}/${framePaths.length} analyzed`);
...
function recordFeedback(sessionId, rating, notes, details = {}) {
  ...
  console.log(`    Ball position: ${(feedback.weights.ballPosition * 100).toFixed(0)}%`);
  console.log(`    Motion peaks:  ${(feedback.weights.motionPeak * 100).toFixed(0)}%`);
  console.log(`    Sharpness:     ${(feedback.weights.sharpness * 100).toFixed(0)}%`);
  console.log(`    Even spacing:  ${(feedback.weights.evenSpacing * 100).toFixed(0)}%`);
...
async function promptFeedback(sessionId, selectedIndices, totalFrames) {
  ...
  console.log(`  Selected frames: [${selectedIndices.join(', ')}] out of ${totalFrames} total`);

  const ratingStr = await ask('  Rate this selection (1=terrible, 5=perfect): ');
  const rating = parseInt(ratingStr) || 3;

  const notes = await ask('  What should be different? (e.g., "missed the crossover peak", "too blurry"): ');

  let moveType = await ask('  What move is this? (dribble/crossover/jumpshot/stepback/etc): ');
  moveType = moveType.trim() || 'unknown';

  let preferredStr = await ask('  Better frame numbers? (comma-separated, or Enter to skip): ');
...
async function analyzeAndSelect(framesDir, outputDir, count, opts = {}) {
  ...
  console.log(`  Selecting: ${count} key frames`);
  ...
  for (let i = 0; i < selectedIndices.length; i++) {
    ...
    const ballStr = a.ball.found
      ...
      : 'no ball';
    console.log(`    [${String(idx).padStart(3)}] score=${a.score.toFixed(3)} motion=${a.motion.toFixed(3)} sharp=${a.sharpness.toFixed(3)} ${ballStr}`);
  }
...
if (require.main === module) {
  ...
  if (!command || !['analyze', 'feedback', 'weights'].includes(command)) {
    ...
    console.log('    analyze <frames-dir> --count 6 --output ./keyframes/');
    console.log('    feedback                       Show current learned weights');
    console.log('    weights --reset                Reset weights to defaults');
    ...
    if (args.includes('--reset')) {
      ...
      if (feedback.sessions.length > 0) {
        ...
        for (const s of recent) {
          console.log(`    [${s.rating}/5] ${s.moveType}: ${s.notes}`);
        }
      }
    }
    process.exit(0);
