...
function buildFilmToSpritePrompt(characterName, animDescription, frameCount, opts = {}) {
  ...
  const prompt = [
    ...
    ``,
    `POSE RULES — CRITICAL, DO NOT DEVIATE:`,
    `- Each frame in the output must match the corresponding frame in Image 1 EXACTLY`,
    `- Treat Image 1 as motion capture data — your job is pixel art TRANSCRIPTION, not artistic interpretation`,
    `- Copy every detail: exact arm angles, exact leg positions, exact spine lean, exact head tilt, exact weight distribution`,
    `- If a pose shows an unusual contortion, extreme reach, or awkward angle — KEEP IT EXACTLY AS IS`,
    `- DO NOT normalize, smooth, straighten, or "correct" any body position`,
    `- DO NOT substitute a more natural-looking pose — the exact contortion is intentional`,
...
function buildFilmToSingleFramePrompt(characterName, animDescription, frameIndex, totalFrames) {
  ...
  const prompt = [
    ...
    `POSE RULES — CRITICAL, DO NOT DEVIATE:`,
    `- Treat Image 1 as motion capture data — your job is pixel art TRANSCRIPTION, not artistic interpretation`,
    `- Copy every detail exactly: arm angles, elbow bends, wrist position, leg stance, knee bend, foot placement, spine lean, shoulder tilt, head angle`,
    `- If the pose shows an unusual contortion, extreme reach, or awkward body position — preserve it EXACTLY`,
    `- DO NOT normalize, smooth, straighten, or "fix" any body position — the exact contortion is intentional`,
    `- DO NOT substitute a more comfortable or natural-looking version of the pose`,
    `- The output pose must be a direct pixel art transcription of what is visible in Image 1`,
    ``,
    `CHARACTER:`,
...
// ─── Direction System Prompts ────────────────────────────────────────────────

const DIRECTION_NAMES = ['Front', 'Front-Right', 'Right', 'Back-Right', 'Back', 'Back-Left', 'Left', 'Front-Left'];
...
/**
 * Build a direction-aware camera/pose hint block for injection into generation prompts.
 * @param {number} dirId 1..8
 * @param {object} [opts]
 * @param {object} [opts.skeletonHints] result from skeletonToPromptHints()
 * @param {string} [opts.animName]
 * @returns {string}
 */
function buildDirectionPrompt(dirId, opts = {}) {
  const { skeletonHints, animName } = opts;
  const label = DIRECTION_NAMES[dirId - 1] ?? `Direction ${dirId}`;
  const cameraHint = DIR_CAMERA_HINTS[dirId] ?? `direction ${dirId} view`;

  const lines = [
    `DIRECTION: d${dirId} (${label})`,
    `Camera angle: ${cameraHint}`,
    `Sprite key suffix: _d${dirId}`,
  ];
  if (skeletonHints?.poseHints) lines.push(`Body pose: ${skeletonHints.poseHints}`);
  if (animName) lines.push(`Animation: ${animName}`);
  return lines.join('\n');
}

/**
 * Inject pre-parsed skeleton pose hints into a prompt string.
 * skeletonHints should be the output of skeletonToPromptHints() for the given dirId.
 * @param {string} promptText
 * @param {object|null} skeletonHints { cameraHint, poseHints }
 * @returns {string}
 */
function injectPoseData(promptText, skeletonHints) {
  if (!skeletonHints) return promptText;
  const lines = [];
  if (skeletonHints.cameraHint) lines.push(`Camera angle: ${skeletonHints.cameraHint}`);
  if (skeletonHints.poseHints) lines.push(`Reference pose: ${skeletonHints.poseHints}`);
