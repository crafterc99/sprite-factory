 ...
 */
function buildSingleFramePrompt(characterName, animationName, frameIndex, totalFrames, opts = {}) {
  const char = CHARACTERS[characterName];
  if (!char) throw new Error(`Unknown character: ${characterName}. Available: ${Object.keys(CHARACTERS).join(', ')}`);
  ...
  // Image 2 = animation pose frame (motion reference)
  // Image 3 = frame 0 style anchor (only present for frames 1+)
  const hasStyleAnchor = opts.hasStyleAnchor === true && frameIndex > 0;
  ...
  const lines = [
    ...
    ``,
    `This is frame ${frameIndex + 1} of ${totalFrames} in a ${anim.action} animation.`,
    `Image 2 (pose to copy) shows: ${frameDesc}`,
    ``,
    `CHARACTER IDENTITY:`,
    `- Keep Image 1's exact character — do not change face, identity, skin tone, hairstyle, or facial structure`,
  ...
  if (hasStyleAnchor) {
    lines.push(
      ``,
      `CONSISTENCY — Image 3 = frame 1 of this animation (already approved):`,
      `- Your output MUST match Image 3's exact pixel art rendering`,
      `- Same line weight, same color palette, same sprite size, same background (#00FF00)`,
      `- This frame must look like it came from the same sprite sheet as Image 3`,
      `- Do NOT change the rendering quality, style, or brightness from Image 3`,
    );
  }
