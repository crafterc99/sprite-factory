 ...
 *
 * The strip is the pose reference Image 1 fed to Nano Banana Pro — the model
 * sees it at the same fidelity we save it. Anything we lose here (aliasing,
 * detail collapse) the model has to invent, which is how generated sprite
 * frames pick up smudged faces, mangled jersey numbers, and ghosted limbs.
 *
 * Defaults:
 *   - lanczos3 kernel for photographic downscale (avoids stair-step aliasing).
 *   - 720px target height — keeps the request payload reasonable while
 *     preserving fine detail from 1080p source frames.
 *   - Caller can override via opts.kernel / opts.targetHeight if the input
 *     is already pixel art and nearest is desired.
 *
 * @param {string[]} framePaths - Paths to frame images
 * @param {string} outputPath - Output PNG path
 * @param {object} opts - { targetHeight, padding, kernel }
 * @returns {{ outputPath, frameCount, width, height }}
 */
async function buildRefStrip(framePaths, outputPath, opts = {}) {
  const targetHeight = opts.targetHeight || opts.height || 720;
  ...
  for (const fp of framePaths) {
    ...
    // Never upscale — if a frame is already smaller than targetHeight, keep
    // it at native resolution. Upscaling photographic content can't add
    // detail and just produces a blurry payload.
    const effectiveHeight = Math.min(targetHeight, meta.height);
    const scale = effectiveHeight / meta.height;
    const newWidth = Math.max(1, Math.round(meta.width * scale));

    const buf = await sharp(fp)
      .resize(newWidth, effectiveHeight, { fit: 'fill', kernel })
      .png({ compressionLevel: 0, effort: 1 })
      .toBuffer();

    resizedBuffers.push(buf);
    frameDims.push({ width: newWidth, height: effectiveHeight });
  }

  // Calculate total strip dimensions — height matches the tallest frame so
  // the canvas can hold every input even if their native sizes vary.
  const totalWidth = frameDims.reduce((sum, d) => sum + d.width + padding, -padding);
  const totalHeight = frameDims.reduce((max, d) => Math.max(max, d.height), 0);
...
if (require.main === module) {
  ...
  if (!inputDir) {
    console.log('\n  Reference Strip Builder\n');
    console.log('  Usage:');
    console.log('    node strip-builder.js <frames-dir> -o <output.png> [--height 512]\n');
    console.log('  Options:');
    console.log('    -o, --output <path>   Output PNG path (default: ref-strip.png)');
    console.log('    --height <px>          Frame height (default: 512)');
    console.log('    --padding <px>         Gap between frames (default: 0)');
    process.exit(0);
  }
