...
function getClient() {
  ...
  _client = new S3Client({
    ...
    // Built-in adaptive retries for transient/throttling errors
    maxAttempts: 4,
    // Reuse TCP/TLS connections across requests — major latency win for
    // sequential operations (restore, backup, batch upload).
...
/** Retry a fn with exponential backoff. The SDK retries individual requests;
 *  this guards whole operations against rarer transient failures.
 *  A missing object (404 / NoSuchKey) is not transient — fail fast. */
async function withRetry(fn, attempts = 3) {
  ...
  for (let i = 0; i < attempts; i++) {
    try { return await fn(); }
      ...
      if (code === 'NoSuchKey' || code === 'NotFound' || status === 404) throw e;
      lastErr = e;
...
async function uploadFile(key, bufferOrPath, contentType = 'image/png') {
  ...
  try {
    ...
    await withRetry(() => client.send(new PutObjectCommand({ Bucket: getBucket(), Key: key, Body, ContentType: contentType })));
  } catch (e) {
...
async function uploadJson(key, obj) {
  ...
  try {
    ...
    await withRetry(() => client.send(new PutObjectCommand({ Bucket: getBucket(), Key: key, Body, ContentType: 'application/json' })));
  } catch (e) {
...
async function downloadFile(key) {
  ...
  try {
    const res = await withRetry(() => client.send(new GetObjectCommand({ Bucket: getBucket(), Key: key })));
    const chunks = [];
