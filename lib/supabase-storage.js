 ...
 * Returns { ok: boolean, keyCount: number, error?: string }
 */
async function verifyConnection() {
  if (!isAvailable()) return { ok: false, error: 'SUPABASE_URL or SUPABASE_SERVICE_KEY not set' };
  try {
    const { data, error } = await getClient().storage.from(BUCKET).list('_meta', { limit: 50 });
    if (error) return { ok: false, error: error.message };
    const totalFiles = await listFiles();
    return { ok: true, keyCount: totalFiles.length, metaKeys: (data || []).map(f => f.name) };
  } catch (e) {
