...
async function removeWhiteBackground(imagePath, outputPath, opts = {}) {
  const threshold = opts.threshold ?? 240; // R,G,B all >= this → transparent
  const feather   = opts.feather   ?? 10;  // pixels within feather distance of edge → semi-transparent
  ...
  // Pass 1: mark pure-white pixels as transparent
  for (let i = 0; i < W * H; i++) {
  ...
  // Pass 2: feather edges — semi-transparent pixels adjacent to transparent pixels
  if (feather > 0) {
    const isBg = (i) => out[i*4+3] === 0;
    for (let y = 0; y < H; y++) {
      for (let x = 0; x < W; x++) {
        const i = y * W + x;
        if (out[i*4+3] === 0) continue; // already transparent
        // Check neighbors within feather distance
