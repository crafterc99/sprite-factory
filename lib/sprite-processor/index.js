 ...
 */
async function placeContentInFrame(transparentPath, bounds, scale, frameSize, padding, outPath) {
  ...
  const resized = await sharp(transparentPath)
    .extract({ left: bounds.minX, top: bounds.minY, width: bounds.w, height: bounds.h })
    .resize(rendW, rendH, { fit: 'fill', kernel: 'nearest' })
    .png({ compressionLevel: 0, effort: 1 })  // lossless, no compression loss
    .toBuffer();
...
async function processFrameSetConsistently(rawPaths, outPaths, opts = {}) {
  ...
  await Promise.all(transparentPaths.map((tp, i) =>
    placeContentInFrame(tp, allBounds[i], scale, frameSize, padding, outPaths[i])
  ));
