...
function _doSync() {
  ...
  const cmd = [
    'git config user.email "autosync@sprite-factory.app" 2>/dev/null || true',
    'git config user.name "Sprite Factory Sync" 2>/dev/null || true',
    `git add ${addTargets}`,
...
// Called on character delete — stages git rm for removed files then syncs
function _gitRmAndSync(filePaths) {
  ...
  const cmd = [
    'git config user.email "autosync@sprite-factory.app" 2>/dev/null || true',
    'git config user.name "Sprite Factory Sync" 2>/dev/null || true',
    `git rm --cached --ignore-unmatch -f ${rmTargets} 2>/dev/null || true`,
    `git add ${addTargets}`,
    'git diff --cached --quiet || git commit -m "data: remove deleted character assets" --no-verify',
    pushCmd,
  ].join(' && ');
  exec(cmd, { cwd: ROOT, timeout: 45000 }, (err, stdout, stderr) => {
    const out = ((stdout || '') + (stderr || '')).trim();
    if (err && out && !out.includes('nothing to commit') && !out.includes('up to date')) {
      console.warn('[auto-git-sync] delete sync failed:', out.slice(0, 300));
    } else {
      console.log('[auto-git-sync] character deletion pushed to git');
    }
  });
}

module.exports = { scheduleSync, _gitRmAndSync };
