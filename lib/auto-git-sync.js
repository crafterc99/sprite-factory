/**
 * Auto Git Sync — no longer used for data persistence.
 *
 * Data (characters, animations, wardrobe, prompts) is now persisted exclusively
 * via Supabase Storage (uploaded on save, restored on startup). Git is used only
 * for source code deploys via Railway.
 *
 * scheduleSync() is kept as a no-op so call sites don't need to change.
 * DATA_TARGETS is empty — nothing is committed to git at runtime.
 *
 * REVERT: to restore git-as-data-store, revert the two commits:
 *   "refactor: untrack all runtime data from git"
 *   "refactor: disable git auto-sync — Supabase is sole data store"
 */
'use strict';
...
// Empty — data is Supabase-only. Source code changes deploy via Railway git push.
const DATA_TARGETS = [];
...
function _getRemoteUrl() {
  ...
  try {
    ...
    if (origin.includes('@')) {
      // Already has credentials embedded — use as-is (Railway may pre-embed token)
      return origin;
    }
...
let _lastSyncResult = { status: 'never', ts: null, error: null };

async function _attemptSync() {
  const remoteUrl = _getRemoteUrl();
  console.log(`[auto-git-sync] starting sync, token=${!!process.env.GITHUB_TOKEN}, remoteUrl=${remoteUrl ? 'set' : 'null'}`);
  ...
  if (!remoteUrl) {
    ...
    if (process.env.GITHUB_TOKEN) {
      console.warn('[auto-git-sync] GITHUB_TOKEN set but could not build remote URL — check git remote');
    } else {
      console.warn('[auto-git-sync] GITHUB_TOKEN not set — data committed locally only');
    }
    _lastSyncResult = { status: 'local-only', ts: new Date().toISOString(), error: 'no push token' };
    return;
  }
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (push.ok) {
      ...
      _lastSyncResult = { status: 'ok', ts: new Date().toISOString(), error: null };
      return;
    }
    ...
    const errMsg = out.slice(0, 300);
    console.warn(`[auto-git-sync] push failed (attempt ${attempt}/3):`, errMsg);
    _lastSyncResult = { status: 'error', ts: new Date().toISOString(), error: errMsg };
...
module.exports = { scheduleSync, _gitRmAndSync, flushSync, getSyncStatus };
